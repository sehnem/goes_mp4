...
def m_filter(files, region):
    drop = []
    for i, file in enumerate(files):
        file_r = files[i]['Key'][-63:-61]
        if region != file_r:
            drop.append(i)
    for index in sorted(drop, reverse=True):
        del files[index]
...
def get_goes(goes, product, region, start=None, end=None, bands=None,
             path='./'):
    ...
    return [path + f['Filename'] for f in files]
